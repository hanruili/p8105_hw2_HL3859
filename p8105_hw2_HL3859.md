...
```
    ...
    ## # A tibble: 584 × 14
    ##    Dumpster Month Year  Date                weight `Volume (cubic yards)`
    ...
    ## #   cigarette_butts <dbl>, `Glass Bottles` <dbl>, `Plastic Bags` <dbl>,
    ## #   Wrappers <dbl>, sports_balls <int>, `Homes Powered*` <dbl>
...
```
    ...
    ## #   cigarette_butts <dbl>, `Plastic Bags` <dbl>, Wrappers <dbl>,
    ## #   `Homes Powered*` <dbl>
...
weight of trash collected (`Weight (tons)`, e.g. 4.31 tons), and the
number of sports balls recovered (`Sports Balls`, e.g. 7). Additionally,
the dataset tracks the number of cigarette butts collected
(`Cigarette Butts`, e.g. 1.26^{5}).
