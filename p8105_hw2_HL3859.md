...
all available records is 216.26 tons. In June 2022, Gwynnda collected a
total of 1.812^{4} cigarette butts.
...
viewers_long <- viewers |>
  ...
  mutate(Series = as.numeric(gsub("Series ", "", Series)))

season_1_avg_viewership <- viewers_long |>
  filter(Series == 1) |>
  summarise(avg_viewership = mean(Viewership, na.rm = TRUE))

season_5_avg_viewership <- viewers_long |>
  filter(Series == 5) |>
  summarise(avg_viewership = mean(Viewership, na.rm = TRUE))

head(viewers_long, 10)
```
...
The average viewership in Season 1 is 2.77; in Season 2 is 10.0.
